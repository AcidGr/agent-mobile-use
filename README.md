...
> 本项目自 **v0.6.0-alpha** 起全面推行语义化版本号（Semantic Versioning）。此前使用的历史版本号体系（如 `v5.2`、`v5.1` 等）已**正式废弃**。原 `v5.2` 版本在语义化演进中等价对应为 `v0.5.2`。当前最新版本为 **`v0.6.3-alpha`**（KSU 模块 versionCode: `603`）。请统一采用新版刷机包与版本规范。
...
1. 从 `release/` 目录或 GitHub Releases 下载最新的刷机包：
   **`agent-mobile-use-ksu-v0.6.3-alpha.zip`**
2. 将 zip 文件传输至手机中。
...
> Starting with **v0.6.0-alpha**, this project officially adopts Semantic Versioning. The legacy version numbering (such as `v5.2`, `v5.1`) is **deprecated**. Former `v5.2` corresponds to `v0.5.2` under SemVer. The current active release is **`v0.6.3-alpha`** (KSU module versionCode: `603`).
